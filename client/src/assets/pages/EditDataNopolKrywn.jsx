    ...
    const getById = async () => {
      try {
        const response = await axios.get(
          `http://localhost:3000/employe/ambilDataById/${id}`
        );
        console.log(response);
        setNama(response.data.data.nama);
        setNo_pol1(response.data.data.no_pol1);
        setNo_pol2(response.data.data.no_pol2);
      } catch (error) {
        console.log(error);
      }
    };
  ...
  const upDateData = async (e) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:3000/employe/UbahData/${id}`,
        {
          nama,
          no_pol1,
          no_pol2,
      ...
      console.log(response);
      toast.success(`Data No Pol karyawan berhasil di Rubah`);
...
export default EditDataNopolKrywn;
